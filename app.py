...
if modulo ==  "Home":
    ...
    # Título de la aplicación
    st.title("Métricas Proyecto de Inversión")
    
    # Descripción del ejercicio
    st.markdown("""
    Esta aplicación permite calcular diferentes métricas para la evaluación de un proyecto de inversión dado.
    """)
        ...
        if st.button("Crear proyecto", type="primary"):
            try:
                if nombre.strip() == "":
                    ...
                    # Guardamos el resumen y las variables privadas
    ...
    with tab_leer:
        ...
        if st.session_state.proyectos_inv:
            ...
            viables = sum(1 for p in st.session_state.proyectos_inv if p.get("decision") == "Viable")
            c1, c2 = st.columns(2)
        ...
        if st.session_state.proyectos_inv:
            ...
            with col1:
                ...
                nueva_tasa = st.number_input(
                    ...
                    value=float(actual["_tasa"]), step=0.5, key=f"tasa_{idx}"
                )
            with col2:
